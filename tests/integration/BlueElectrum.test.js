...
describe('BlueElectrum', () => {
  ...
  it('ElectrumClient can test connection', async () => {
    assert.ok(
      !(await BlueElectrum.testConnection(
    ...
    assert.ok(
      !(await BlueElectrum.testConnection('joyreactor.cc', false, 443))
    );
    assert.ok(
      !(await BlueElectrum.testConnection('joyreactor.cc', 443, false))
    );
    ...
    assert.ok(
      await BlueElectrum.testConnection('electrum1.malinwallet.io', '50001'),
    )
    assert.ok(
      await BlueElectrum.testConnection('electrum1.malinwallet.io', false, 443)
    );
  })
  ...
  it.skip('BlueElectrum can do getMempoolTransactionsByAddress()', async function () {
    const txs = await BlueElectrum.getMempoolTransactionsByAddress(
  ...
  it.each([false, true])(
    'BlueElectrum can do multiGetBalanceByAddress(), disableBatching=%p',
